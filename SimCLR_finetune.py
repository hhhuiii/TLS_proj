...
#     def forward(self, x):
#         _, (h, _) = self.encoder(x)  # 会返回[output,[h，c]]，output是每个时间步的输出，h，c是最后一个时间步的隐藏状态和细胞状态
#         h = h.squeeze(0)  # 提取LSTM的最后隐藏状态
#         out = self.classifier(h)  # 使用分类头进行分类
#         return out  # 返回每个类别的预测分数
...
class FineTuneModel(nn.Module):
    ...
    def forward(self, x):
        # output, (h, c) = self.encoder(x)  # output: [batch_size, seq_len, hidden_dim]

        # 注意力机制
        # attn_scores = self.attn(output)  # [batch_size, seq_len, 1]
        # attn_weights = torch.softmax(attn_scores, dim=1)  # [batch_size, seq_len, 1]
        # context = torch.sum(attn_weights * output, dim=1)  # [batch_size, hidden_dim]
...
# 不同encoder结构的微调模型
# class FineTuneModel(nn.Module):
#     def __init__(self, pre_trained_encoder, num_classes=NUMCLASS):
#         super(FineTuneModel, self).__init__()
#         self.encoder = pre_trained_encoder
#         self.classifier = nn.Linear(512, num_classes)  # 因为双向LSTM拼接了两个256

#     def forward(self, x):
#         _, (h_n, _) = self.encoder(x)
#         h_forward = h_n[-2, :, :]  # 最后一层正向
#         h_backward = h_n[-1, :, :]  # 最后一层反向
#         h = torch.cat((h_forward, h_backward), dim=1)  # [batch_size, 512]
...
def evaluate(csv_test, pre_trained_encoder, model_path='finetuned_model.pt', output_dir='./finetune_output'):
    ...
    raw_embedding = reducer_raw.fit_transform(raw_inputs_vis)
    ...
    # 编码器输出降维可视化
    reducer_feat = umap.UMAP(n_components=2, random_state=42)
    feature_embedding = reducer_feat.fit_transform(features_vis)
...
if __name__ == '__main__':
    ...
    # 直接加载LSTM而不是封装类
    # encoder = nn.LSTM(input_size=1, hidden_size=256, batch_first=True)
    ...
    encoder = LSTMEncoderWithAttention()
